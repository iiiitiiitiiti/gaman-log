...
- **がまん**: 今月がまんした金額と、それで買えた身近なモノ（ラーメン○杯ぶん など）を表示します。連続でがまんした日数と、目標（ほしいもの）までのゲージも出します
- **むだづかい**: 今月のむだづかいを警報ふうに表示し、「このままだと1年で○円消えます」と知らせます。月の上限を決めると残りを出し、超えると警報が強くなります
- **ふりかえり**: 月ごとの合計・直近6か月の推移・よくがまんした／使ってしまった品目のランキング、自慢用の画像づくり
- **設定**: 目標・月の上限・定番ボタン（並べ替え可）・換算表の編集と、データの書き出し・読み込み
- がまん・むだづかいの画面の上部に「がまん − むだづかい」の収支を出します
- 1年ペースの予測は、記録を始めて7日目から出します（直近30日、または記録を始めてからの日数で1日あたりにならして365倍）
- 履歴をタップすると、品名・金額・日時を直せます
- 前回の書き出しから30日たつと、書き出しを勧めるお知らせを出します
...
| `src/equivalents.ts` | 金額を身近なモノに置き換える目安の表（設定で編集した表があればそちら） |
| `src/ReviewView.tsx` / `src/shareImage.ts` | ふりかえりタブと自慢用画像（canvas で PNG を作る） |
